...
def _check_llm_provider(stage_name: str, stage_cfg, model_pricing: dict) -> tuple[bool, str, str]:
    missing = []
    ...
    if pricing:
        pricing_str = f"${pricing.input_per_million}/{pricing.output_per_million} per 1M"
    else:
...
def doctor(config_path: str) -> None:
    ...
    for stage in LLM_STAGES:
        ...
        name = stage.title()
        ok, reason, pricing_str = _check_llm_provider(stage, cfg, settings.model_pricing)
        if not ok:
            all_ok = False
        if ok:
            stage_lines.append(f"  {click.style('✔', fg='green')} {name} provider  {pricing_str}")
        else:
            stage_lines.append(f"  {click.style('✖', fg='red')} {name} provider: {reason}")
    ...
    tavily_icon = click.style("✔", fg="green") if tavily_ok else click.style("✖", fg="red")
