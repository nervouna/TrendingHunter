...
Secrets are stored in `.env` (gitignored) and referenced via `${VAR_NAME}` syntax in `config.yaml`. The config loader supports env var overrides without editing YAML: use `TH__*` with double-underscore `__` as a level separator (e.g. `TH__LLM__DRAFT__MODEL=gpt-4o`), or the legacy `TH_*` form (e.g. `TH_LLM_DRAFT_MODEL=gpt-4o`).
